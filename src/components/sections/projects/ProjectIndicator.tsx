...
import { TAB_NUMBER_OFFSET, TAB_INDEX_INACTIVE } from './project.constants';
...
const IndicatorDot = memo(
  ({ index, isActive, onSelect }: IndicatorDotProps) => (
    <button
      type='button'
      role='tab'
      aria-label={`Go to project ${index + TAB_NUMBER_OFFSET}`}
      aria-selected={isActive}
      aria-current={isActive ? 'true' : undefined}
      tabIndex={isActive ? 0 : TAB_INDEX_INACTIVE}
